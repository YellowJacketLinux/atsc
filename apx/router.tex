...
\textmdef{Ideal Router}. This uplink cable should be \qtyrange{23}{24}{\awg} solid copper so that it properly
...
For MoCA, DirecTV SWM receivers have built-in MoCA clients that use the `E-Band' (\qtyrange{400}{700}{\mega\hertz}).
...
Note that some WAPs are now capable of speeds beyond \qty{1}{\giga\bit\per\second}. If those access points
have a \qty{2.5}{\giga\bit\per\second} Ethernet jack and is uplinking through MoCA, then it may be worth the
extra money to buy a \moca{2.5} adapter that also has a \qty{2.5}{\giga\bit\per\second} Ethernet jack.
...
prior to \wifi{6e} support WiFi operation on both the \qty{2.4}{\giga\hertz} and \qty{5}{\giga\hertz} bands.
...
\wifi{6e} (\ieee{802.11ax} extended) and \wifi{7} (\ieee{802.11be}) also support the \qty{6}{\giga\hertz} band.
...
would not also see from less expensive \wifi{6} \emph{without} the \qty{6}{\giga\hertz} band. The vast majority
...
\myhref{https://antronix.myshopify.com/products/cmc2002ha}{CMC2002HA} (or equivalent \qty{1002}{\mega\hertz})
passive splitter should be used.

For \docsis{3.1} `Phase 1', an Antronix
\myhref{https://antronix.myshopify.com/products/cmc4002h}{CMC4002H} (or equivalent \qty{1218}{\mega\hertz})
passive splitter should be used.

For \docsis{3.1} `Phase 2' or \docsis{4.0}, an Antronix
\myhref{https://rb.gy/542gd}{CMC5002H-A} (or equivalent \qty{1800}{\mega\hertz})
